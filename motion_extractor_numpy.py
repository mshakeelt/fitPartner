...
def get_keypoints():
    ...
    video_height = int(front_cam.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    while(True):
        ...
        if front_ret and right_ret and left_ret:
            ...
            xyz_coordinates = inference.get_xyz_coordinates(current_frame_coordinates, previous_frame_coordinates, video_height)
            if first_stack:
                if not None in xyz_coordinates.values():
                    previous_frame_coordinates = current_frame_coordinates
                    video_keypoints.append(xyz_coordinates.values())
                    first_stack = False
            else:
                previous_frame_coordinates = current_frame_coordinates
                video_keypoints.append(xyz_coordinates.values())
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                print("Recording stopped, moving on to writing bvh file!")
                break
        else:
...
def warmup():
    ...
    right_cam = cv2.VideoCapture(1, cv2.CAP_DSHOW)
    left_cam = cv2.VideoCapture(2, cv2.CAP_DSHOW)
    print("Camera testing started. Make the starting pose and press s to start recording! \n")
    while(True):
        front_ret, front_frame = front_cam.read()
        right_ret, right_frame = right_cam.read()
        left_ret, left_frame = left_cam.read()
        if front_ret and right_ret and left_ret:
            ...
            if cv2.waitKey(1) & 0xFF == ord('s'):
                print("Exitting the setup and starting to record the motion!")
                break
        else:
...
if __name__ == '__main__':
    ...
    warmup()
    video_keypoints = get_keypoints()
    if video_keypoints:
        video_keypoints = np.stack(video_keypoints)
        output_file_name = 'test_with_rotation_calculation_modified.bvh'
        video_keypoints = np.load(os.path.join("output", "test_with_rotation_calculation.npy"))
    else:
        print("Faild to find a single frame with all keypoints!!!")
        print("Exitting!")